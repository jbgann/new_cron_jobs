...
class Cron_Job:
    def __init__(self,name,commands):
        ...
        self.config_dict = getConfigAsDict()


    def runCommand(self,command):
        success = runCommandWithLogging(command,
                                        self.name,
                                        "10s",
                                        self.config_dict["cron_log"])
        if not success : exit(1)

    def runCommandsInConfigContext(self):
        for command in self.commands:
            pprint.pprint(self.config_dict)
            self.runCommand(command.format(**self.config_dict))
